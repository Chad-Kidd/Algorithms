...
#sell at high number
testing_price_list = [78, 2, 3, 0, 75, 86, 107]
...
def find_max_profit(prices): #receives as input a list of stock prices
  ...
  for i in range(1, len(prices)): #pass prices
    ...
    p = 0
    print("this is p", p) #loops
    while p < i:
      if prices[i] - prices[p] > max_profit:
        max_profit = prices[i] + prices[p]
        #suck at math logic 
        # not sure if supposed to be + or -

      p +=1 #needed to continue iterating otherwise breaks
  return max_profit

print(find_max_profit(testing_price_list)) #109 TESTED WORKING
